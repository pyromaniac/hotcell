...
describe PufferMarkup::Block do
  def method_missing method, *args, &block
    klass = PufferMarkup::Calculator::HANDLERS[method] ?
      PufferMarkup::Calculator : PufferMarkup::Node

    instance = PufferMarkup::Assigner.new *args if method == :ASSIGN
    instance = PufferMarkup::Summoner.new *args if method == :METHOD
    ...
    klass = PufferMarkup::Tag if method == :TAG
    instance = PufferMarkup::Command.new *args if method == :COMMAND
    instance = PufferMarkup::Block.new *args if method == :BLOCK
    klass = PufferMarkup::Joiner if method == :JOINER
  ...
  describe 'complex parsing and rendering' do
    ...
    let(:if_tag) do
      Class.new(described_class) do
        ...
        def validate!
          ...
          raise PufferMarkup::Errors::BlockError.new 'Invalid if syntax' unless valid
        end
    ...
    before { PufferMarkup.stub(:commands) { {} } }
    before { PufferMarkup.stub(:blocks) { { 'if' => if_tag } } }
    before { PufferMarkup.stub(:subcommands) { { 'elsif' => if_tag, 'else' => if_tag } } }
    ...
    context do
      ...
      }.to raise_error PufferMarkup::Errors::BlockError }
    end
  end
